 ...
 *
 * @param {object} currentData - The currently displayed item (used to resolve anatomy role descriptions)
 * @returns {{ anatomyMode, setAnatomyMode, tooltipRef, tooltipTitleRef, tooltipRoleRef, handleMouseMove }}
 */
export function useAnatomyMode(currentData) {
  const [anatomyMode, setAnatomyMode] = useState(() => {
    return localStorage.getItem('anatomyMode') === 'true';
  });

  const tooltipRef = useRef(null);
  const tooltipTitleRef = useRef(null);
  const tooltipRoleRef = useRef(null);
  const hoveredNodeRef = useRef(null);

  // Persist anatomyMode to localStorage
  useEffect(() => {
    localStorage.setItem('anatomyMode', anatomyMode);
  }, [anatomyMode]);
  ...
  const handleMouseMove = (e) => {
    ...
    const layer = element?.closest('.anatomy-layer');
